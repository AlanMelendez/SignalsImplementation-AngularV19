...
private contacts: Contact[] = [
  ...
  private generateUniqueId(): string {
    const existingIds = this.contacts.map((c) => c.id); // Get onlu the IDs from the contacts array.
    const maxId = Math.max(...existingIds); //Get the last ID in the array.
    return (maxId + 1).toString(); // Return the next ID.
  }
  ...
  async addContact(contact: Contact): Promise<Contact> {
    ...
    const newContact = {
      ...contact,
      id: Number(this.generateUniqueId()), // Rewrite the ID with a new random ID.
    };
